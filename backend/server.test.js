...
const loadServer = async ({
  ...
  process.env.RATE_LIMIT_MAX_REQUESTS_DOCENTE = String(
    rateLimitMaxRequestsDocente,
  );
  process.env.RATE_LIMIT_MAX_REQUESTS_COORDINADOR = String(
    rateLimitMaxRequestsCoordinador,
  );
  process.env.RATE_LIMIT_MAX_REQUESTS_ADMIN = String(rateLimitMaxRequestsAdmin);
  process.env.RATE_LIMIT_NEAR_THRESHOLD_RATIO = String(
    rateLimitNearThresholdRatio,
  );
  process.env.RATE_LIMIT_BUCKET_CLEANUP_INTERVAL_MS = String(
    rateLimitCleanupIntervalMs,
  );
  process.env.RATE_LIMIT_MAX_BUCKETS = String(rateLimitMaxBuckets);
  process.env.RATE_LIMIT_BUCKET_COUNT_LOG_INTERVAL_MS = String(
    rateLimitBucketCountLogIntervalMs,
  );
...
describe("backend/server API", () => {
  ...
  it("GET /api/rubricas lista plantillas vigentes por materia/grado", async () => {
    ...
    try {
      ...
      const result = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/rubricas?materia=Matem%C3%A1ticas&grado=6to%20primaria",
      });
      expect(result.status).toBe(200);
  ...
  it("CRUD /api/rosters administra listas de grupo con estudiantes", async () => {
    const app = await loadServer({ provider: "openai" });
    ...
    try {
      const created = await apiRequest({
        ...
        body: {
          ...
          term: "2026-T1",
          students: [
            { nombre: "Ana Pérez", matricula: "2026001" },
            { nombre: "Luis Soto", matricula: "2026002" },
          ],
        },
      });
      ...
      expect(created.json.data.students).toHaveLength(2);

      const listed = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/rosters?group=6A&term=2026-T1",
      });
      expect(listed.status).toBe(200);
      expect(listed.json.data).toHaveLength(1);

      const rosterId = created.json.data.id;
      const updated = await apiRequest({
        baseUrl: app.baseUrl,
        path: `/api/rosters/${encodeURIComponent(rosterId)}`,
        method: "PUT",
        body: {
          group: "6A",
          term: "2026-T2",
          students: [{ nombre: "Ana Pérez", matricula: "2026001" }],
        },
      });
      expect(updated.status).toBe(200);
      expect(updated.json.data.term).toBe("2026-T2");
      expect(updated.json.data.students).toHaveLength(1);
          ...
          choices: [
            {
              message: {
                content:
                  '{"puntuacion_sugerida": 88, "justificacion_breve": "Correcto"}',
              },
            },
          ],
        }),
  ...
  it("POST /api/calificacion/sugerir hace failover a secundario cuando falla primario", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse(502, { error: { message: "upstream fail" } }),
      )
      .mockResolvedValueOnce(
        jsonResponse(200, {
          choices: [
            {
              message: {
                content:
                  '{"puntuacion_sugerida": 91, "justificacion_breve": "Fallback ok"}',
              },
            },
          ],
        }),
    ...
    try {
      ...
      const failoverLogged = logSpy.mock.calls.some(([line]) =>
        String(line).includes('"event":"provider_failover"'),
      );
  ...
  it("actualiza métricas de /api/calificacion/sugerir para éxito y error", async () => {
          ...
          choices: [
            {
              message: {
                content:
                  '{"puntuacion_sugerida": 89, "justificacion_breve": "OK"}',
              },
            },
          ],
        }),
    ...
    try {
      ...
      expect(
        metrics.json.endpoints["/api/calificacion/sugerir"].error_rate,
      ).toBeCloseTo(0.5, 6);
      expect(
        metrics.json.endpoints["/api/calificacion/sugerir"].latency_ms.samples,
      ).toBe(2);
      expect(
        metrics.json.endpoints["/api/calificacion/sugerir"].latency_ms.p95,
          ...
          choices: [
            {
              message: {
                content:
                  '{"puntuacion_sugerida": 77, "justificacion_breve": "Prom"}',
              },
            },
          ],
        }),
    ...
    try {
      ...
      const updated = await apiRequest({
        ...
        body: {
          ...
          calificacionFinal: {
            ...created.json.calificacionFinal,
            notaSobre100: 91,
            letra: "A",
          },
        },
      ...
      const rawLogs = execFileSync(
        ...
        [
          "-json",
          process.env.REPORTS_DB_FILE,
          "SELECT action, actor, metadata_json FROM audit_logs ORDER BY created_at ASC;",
        ],
        { encoding: "utf-8" },
      );
      ...
      expect(JSON.parse(auditLogs[2].metadata_json)).toMatchObject({
        previousFinalGrade: expect.objectContaining({
          notaSobre100: 80,
          letra: "B",
        }),
        nextFinalGrade: expect.objectContaining({
          notaSobre100: 91,
          letra: "A",
        }),
      });
    ...
    try {
      ...
      const rawLogs = execFileSync(
        ...
        [
          "-json",
          process.env.REPORTS_DB_FILE,
          "SELECT action, actor, metadata_json FROM audit_logs WHERE action = 'report_created' ORDER BY created_at DESC LIMIT 1;",
        ],
        { encoding: "utf-8" },
      );
  ...
  it("permite acceder reportes dentro del mismo tenant", async () => {
    const app = await loadServer({ provider: "openai" });
    const ownerToken = buildSessionToken({
      sub: "u-docente-owner",
      role: "docente",
      tenantId: "tenant-1",
      institution: "inst-1",
    });
    const tenantCoordinatorToken = buildSessionToken({
      sub: "u-coord-1",
      role: "coordinador",
      tenantId: "tenant-1",
      institution: "inst-1",
  ...
  it("versiona reportes al actualizar y persiste diff mínimo de calificacionFinal", async () => {
    const app = await loadServer({ provider: "openai" });
    const docenteToken = buildSessionToken({
      sub: "u-docente-version",
      role: "docente",
      tenantId: "tenant-v",
      institution: "inst-v",
    });
    ...
    try {
      ...
      expect(
        versions.json.data.map((version) => version.versionNumber),
      ).toEqual([2, 1]);
  ...
  it("aísla consultas de versiones por tenant y usuario", async () => {
    ...
    const sameTenantDifferentTeacher = buildSessionToken({
      sub: "u-doc-other",
      role: "docente",
      tenantId: "tenant-1",
      institution: "inst-1",
    });
    const foreignTenantCoordinator = buildSessionToken({
      sub: "u-coord-foreign",
      role: "coordinador",
      tenantId: "tenant-2",
      institution: "inst-2",
  ...
  it("deniega acceso cross-tenant por id y export con 403", async () => {
    ...
    const ownerToken = buildSessionToken({
      ...
      institution: "inst-owner",
    });
    const foreignTenantToken = buildSessionToken({
      sub: "u-auditor-foreign",
      role: "auditor",
      tenantId: "tenant-foreign",
      institution: "inst-foreign",
    });
  ...
  it("aísla listados por tenant y por usuario cuando aplica política", async () => {
    const app = await loadServer({ provider: "openai" });
    const docenteA = buildSessionToken({
      sub: "u-docente-a",
      role: "docente",
      tenantId: "tenant-1",
      institution: "inst-1",
    });
    const docenteB = buildSessionToken({
      sub: "u-docente-b",
      role: "docente",
      tenantId: "tenant-1",
      institution: "inst-1",
    });
    const docenteOtherTenant = buildSessionToken({
      sub: "u-docente-c",
      role: "docente",
      tenantId: "tenant-2",
      institution: "inst-2",
    });
    const auditorTenant1 = buildSessionToken({
      sub: "u-auditor-1",
      role: "auditor",
      tenantId: "tenant-1",
      institution: "inst-1",
    ...
    try {
      const createReportFor = (authToken, nombre) =>
        apiRequest({
          baseUrl: app.baseUrl,
          path: "/api/reportes",
          method: "POST",
      ...
      expect((await createReportFor(docenteOtherTenant, "Carla")).status).toBe(
        201,
      );

      const listedDocenteA = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/reportes",
        authToken: docenteA,
      });
      ...
      const listedAuditorTenant1 = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/reportes",
        authToken: auditorTenant1,
      });
      expect(listedAuditorTenant1.status).toBe(200);
      expect(listedAuditorTenant1.json.data).toHaveLength(2);
      expect(
        listedAuditorTenant1.json.data
          .map((report) => report.estudiante.nombre)
          .sort(),
      ).toEqual(["Ana", "Beto"]);
    } finally {
      await app.close();
  ...
  it("devuelve 403 cuando el rol no tiene permiso para exportar", async () => {
    ...
    try {
      ...
      const correctorToken = buildSessionToken({
        role: "corrector",
        sub: "u-corrector-1",
  ...
  it("permite exportar cuando el rol tiene permiso", async () => {
    ...
    try {
      ...
      const coordinadorToken = buildSessionToken({
        role: "coordinador",
        sub: "u-coord-1",
  ...
  it("aplica RBAC por endpoint protegido según rol", async () => {
    ...
    const adminAliasToken = buildSessionToken({
      role: "  Administrador  ",
      sub: "u-admin-alias-1",
    });
    const docenteToken = buildSessionToken({
    ...
    const auditorToken = buildSessionToken({
      role: "auditor",
      sub: "u-auditor-1",
    });
  ...
  it("audita denegaciones 401/403 por rol y recurso", async () => {
    ...
    const foreignAuditorToken = buildSessionToken({
      role: "auditor",
      sub: "u-foreign-auditor",
      tenantId: "tenant-foreign",
      institution: "inst-foreign",
    });
    const foreignCoordinatorToken = buildSessionToken({
      role: "coordinador",
      sub: "u-foreign-coord",
      tenantId: "tenant-foreign",
      institution: "inst-foreign",
    ...
    try {
      ...
      expect(
        rawLogs.some((line) =>
          line.includes('"event":"authentication_failed"'),
        ),
      ).toBe(true);
      expect(
        rawLogs.some(
          (line) =>
            line.includes('"event":"authorization_denied"') &&
            line.includes('"resource":"/api/calificacion/sugerir"'),
        ),
      ).toBe(true);
      expect(
        rawLogs.some(
          (line) =>
            line.includes('"event":"report_scope_denied"') &&
            line.includes('"endpoint":"/api/reportes/:id/export"'),
        ),
      ).toBe(true);
    } finally {
      await app.close();
  ...
  it("DELETE /api/reportes/:id elimina de forma segura y audita report_deleted", async () => {
    ...
    try {
      ...
      const deletedAudit = auditLogs.find(
        (log) => log.action === "report_deleted",
      );
  ...
  it("deleteReportGraph revierte borrado si falla la inserción de auditoría", async () => {
    ...
    try {
      ...
      const existingAudit = JSON.parse(
        execFileSync(
          ...
          [
            "-json",
            process.env.REPORTS_DB_FILE,
            `SELECT id FROM audit_logs WHERE report_id = '${created.json.id}' LIMIT 1;`,
          ],
          { encoding: "utf-8" },
        ),
      )[0];
      ...
      await expect(
        storage.deleteReportGraph(created.json.id, [
          {
            id: existingAudit.id,
  ...
  it("aplica rate limiting por usuario dentro del mismo tenant", async () => {
    ...
    try {
      ...
      const otherUser = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: docenteB,
  ...
  it("aplica límites diferenciados por rol y registra saturación", async () => {
    ...
    try {
      ...
      const docenteFirst = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: docenteToken,
      ...
      const docenteSecond = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: docenteToken,
      ...
      const coordFirst = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: coordinadorToken,
      ...
      const coordSecond = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: coordinadorToken,
      ...
      const coordThird = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: coordinadorToken,
      ...
      const adminFirst = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminToken,
      ...
      const adminSecond = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminToken,
      ...
      const adminThird = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminToken,
      ...
      const adminFourth = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminToken,
      ...
      const adminAliasFirst = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminAliasToken,
      ...
      const adminAliasSecond = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminAliasToken,
      ...
      const adminAliasThird = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminAliasToken,
      ...
      const adminAliasFourth = await apiRequest({
        baseUrl: app.baseUrl,
        path: "/api/calificacion/sugerir",
        method: "POST",
        authToken: adminAliasToken,
      ...
      const saturationLogged = logSpy.mock.calls.some(([line]) =>
        String(line).includes('"event":"rate_limit_saturation"'),
      );
  ...
  it("expone utilidades unitarias para limpieza y descarte LRU simple", async () => {
    const { cleanupExpiredBuckets, enforceBucketCapacity } =
